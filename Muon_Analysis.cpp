...
//void Muon_Analysis(string inputFile, string ofile);
//int main(int argc, char** argv){
//
//    string inputFile = argv[1];
//    string outputFile = argv[2];
//    Muon_Analysis(inputFile, outputFile);
//}
...
void Muon_Analysis(string inputFile, string ofile){
    ...
    // define hist for muon number
    TH1I *h_muon_num = new TH1I("h_muon_num", "h_muon_num", 10, -1, 9);
    ...
    for (UInt_t i = 0; i < nEv; i++){
        ...
        // rquire to have two muons with pt > 30 and abs(eta) < 2.4 and tightId and to be isolated
        bool muon_selection = (Muon_pt[0]>30. && abs(Muon_eta[0])<2.4 && Muon_tightId[0] && Muon_pfRelIso04_all[0] < 0.15);  
        muon_selection = muon_selection && (Muon_pt[1]>30. && abs(Muon_eta[1])<2.4 && Muon_tightId[1] && Muon_pfRelIso04_all[1] < 0.15);
        ...
        bool selection = muon_selection && (Muon_charge[0] * Muon_charge[1]) < 0;
