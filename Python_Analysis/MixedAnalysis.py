...
def make_parser():
    ...
    parser.add_argument("-s", "--sum_w", type=float, help="Sum of the weights\
of this sample. Summed across all datset files.")
...
def skimming(filename, ofilename, xs=None, lumi=None, mc_flag=False,
    ...
    for events in tree.iterate(
        ...
        events['N_jet_medium'] = ak.sum(
            events['Jet_btagDeepFlavB'] > btag_deepflav_wp['medium'], axis=1)
        events['N_jet_tight'] = ak.sum(
            events['Jet_btagDeepFlavB'] > btag_deepflav_wp['tight'], axis=1)
